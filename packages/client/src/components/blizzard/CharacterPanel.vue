<template>
  <div>
    ...
    <v-card
      :class="`left-border-rounded character class-border-${character.class.id} mb-4`"
    >
      <v-layout wrap>
        <v-row
          no-gutters
          :class="`character--bg class-logo-${character.class.id}`"
        >
            ...
            >
              <div>
                <v-img
                  ...
                  height="84"
                  width="84"
                  :class="`thumbnail class-color-${character.class.id}`"
                />
              </div>

              <h2 class="character--name mt-2">{{ character.name }}</h2>
              <span class="character--info mb-2">
                {{ character.specialization.name }}
                {{ character.class.name }}</span
              >
              ...
              <div class="character--links">
                ...
                >
                  <v-img
                    src="/images/svg/warcraftlog-logo.png"
                    width="30"
                    height="30"
                    style="margin: 0 7px"
                  />
          ...
          <v-col class="character--bg__clear pa-4">
            <h3
              ...
              :class="`class-color-${character.class.id}`"
            >
              Raid Progression
            </h3>
          ...
          <v-col class="character--bg__clear pa-4">
            ...
            <v-row>
              <v-col
                ...
                :key="score.season"
                class="character--raiderIO"
              >
                <div
                  :class="`character--raiderIO--score class-color-${character.class.id}`"
                >
          ...
          <v-col
            ...
            lg="12"
            xl="4"
            class="character--bg__clear pa-4"
          >
            <h3
              class="character--header"
              :class="`class-color-${character.class.id}`"
            ...
            >
              <div v-for="item in character.equipment" :key="item.item.id">
                ...
                >
                  <v-img
                    :src="
                      item.media && item.media.assets && item.media.assets.value
      ...
      <v-layout>
        <v-row no-gutters>
          <v-col>
            <div class="character--footer">
              <span v-if="!applying" class="character--footer__text">{{
                lastUpdated
              }}</span>
              <v-btn
                v-else-if="typeof order === 'number' && order !== 0"
                text
                @click="setMain"
              >
                Set as Main Character
              </v-btn>
              <v-btn v-if="applying" text @click="removeCharacter"
...
import { useRaids, useSubmission } from '@client/stores'
import { computed, defineComponent, PropType } from '@nuxtjs/composition-api'
import { EquippedItem } from '@server/blizzard/interfaces/profile/character-equipment/character-equipment-summary.interface'
...
export default defineComponent({
  props: {
    character: {
      type: Object as PropType<FormCharacter>,
      required: true,
    },
    applying: {
      type: Boolean,
  ...
  setup(props) {
    ...
    const lastUpdated = computed(() => {
      ...
      const updatedAt = new Date(props.character.updatedAt)
      const now = new Date()
    ...
    const scores = computed(() => {
      ...
      const seasons = props.character.raiderIO?.mythic_plus_scores_by_season
    ...
    const progression = computed(() => {
      if (!raids.value.length || !props.character.raiderIO?.raid_progression)
        return []

      const retval: ComputedRaid[] = []

      for (const raid of raids.value) {
        const data = props.character.raiderIO.raid_progression[raid.slug]
    ...
    const avatar = computed(() => {
      if (props.character?.media?.avatar) {
        return props.character.media.avatar
      } else if (props.character.race_id && props.character.gender) {
        return `https://render-us.worldofwarcraft.com/shadow/avatar/${
          props.character.race_id
        }-${props.character.gender === 'Female' ? 1 : 0}.jpg`
      }

      return `https://render-us.worldofwarcraft.com/shadow/avatar/10-${Math.round(
        Math.random()
      )}.jpg`
    })
    ...
    const setMain = () => {
      if (typeof props.order === 'number') {
        submissionStore.characters.unshift(
          submissionStore.characters.splice(props.order, 1)[0]
        )
      }
    }
    ...
    const removeCharacter = () => {
      if (typeof props.order === 'number') {
        submissionStore.characters.splice(props.order, 1)
      }
    }

    const wowheadTooltip = (classId: number, item: EquippedItem) => {
      let str = ''
      ...
      if (item.azerite_details && item.azerite_details.selected_powers) {
        str += `&azerite-powers=${classId}:${item.azerite_details.selected_powers
          .map((p) => p.id)
          .join(':')}`
      }
