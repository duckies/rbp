<template>
  ...
  >
    <v-img
      class="character--bg__image card-expand"
      :src="media.main"
      height="280"
    >
      <div class="character--content">
        ...
        <div class="character--info">
          ...
          <div class="character--info__rank" v-text="rank" />
        </div>
      </div>
...
export default defineComponent({
  props: {
    character: {
      type: Object as PropType<GuildCharacter>,
      required: true,
    },
  },
  setup(props) {
    const specIcon = computed(() => props.character.specialization.media.value)
    const media = computed(() => props.character.media)

    const titledName = computed(() => {
      const name = `<span class="class-color-${props.character.class.id}"><strong>${props.character.name}</strong></span>`

      if (!props.character.title) return name

      return props.character.title.replace('{name}', name)
    })

    const rank = computed(() => {
      return typeof props.character.guild_rank === 'number'
        ? Ranks[props.character.guild_rank]
        : 'none'
    })

    const armoryLink = computed(
      () =>
        `https://www.worldofwarcraft.com/en-us/character/us/${
          props.character.realm
        }/${props.character.name.toLowerCase()}`
    )

    const raiderIOLink = computed(
      () =>
    ...
    const wclLink = computed(
      () =>
        `https://www.warcraftlogs.com/character/${props.character.region}/${
          props.character.realm
        }/${props.character.name.toLowerCase()}`
    )
