    ...
    <div class="character--content">
      <div class="character--avatars">
        <v-img
          ...
          height="90"
          width="90"
          :class="'character--avatar class-border-' + character.class"
        />
        <v-img
          :src="specIcon"
          height="33"
          width="33"
          :class="'character--spec class-border-' + character.class"
        />
      </div>
...
export default class CharacterWindow extends Vue {
  ...
  get background(): string {
    return this.thumbnail.replace('avatar', 'main')
  }

  get specIcon(): string {
    return `http://render-us.worldofwarcraft.com/icons/56/${this.character.specIcon}.jpg`
  }

  get titledName(): string {
    const name = `<span class="class-color-${this.character.class}">${this.character.name}</span>`

    if (!this.character.title) return name

    return this.character.title.replace('%s', name)
  }

  get rank(): string {
    return Ranks[this.character.guildRank]
  }
